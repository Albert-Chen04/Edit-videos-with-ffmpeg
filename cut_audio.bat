...
set "input_file=E:\48tool\pocket48\pocket48-live-broadcast-replay\20250703\B站\[口袋48录播]_CGT48-谭思慧_☁️_2025-07-03~23.45.28_1147678717646802944__2025-07-05~07.33.39.ts"
set "output_dir=E:\48tool\pocket48\pocket48cut\20250703\音频"
...
for /l %%i in (0,1,16) do (
    for /f "tokens=1-3" %%a in ("!clips[%%i]!") do (
        ...
        REM -ss [开始时间] -t [持续时长]
        REM -vn: 忽略视频流
        REM -c:a copy: 直接复制音频流，不进行重新编码，速度快且无损
        ffmpeg -nostdin -ss !start! -i "%input_file%" -t !duration! -vn -c:a copy -y "%output_dir%\!name!.m4a"
        if errorlevel 1 (
            echo 错误: 提取 !name! 失败
        ) else (
            echo 成功: !name! 已保存
